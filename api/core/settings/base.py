...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        "auth": "10/min",  # brute-force guard on login/register/password-reset — see users/throttles.py
    },
}
...
# Required for the refresh-token cookie (credentials: "include") to survive the
# cross-origin fetch between the Next.js app and this API in dev/prod.
CORS_ALLOW_CREDENTIALS = True

# ── Auth cookie / frontend link config ────────────────────────────────────────
# Blank in dev (host-only cookie, works fine across localhost ports). Set to
# ".kiphaus.com" in prod so the refresh cookie is visible to both app.kiphaus.com
# and api.kiphaus.com.
...
DEFAULT_FROM_EMAIL = env("DEFAULT_FROM_EMAIL", default=f"Kiphaus <{EMAIL_HOST_USER}>" if EMAIL_HOST_USER else "Kiphaus <noreply@gmail.com>")
...
CELERY_BEAT_SCHEDULE = {
    ...
    "send-checkin-reminders": {
        "task":     "notifications.tasks_scheduled.send_checkin_reminders",
